<template>
  <div>
    ...
    <div v-if="($page.flash.error || Object.keys($page.props.errors).length > 0) && show" class="mb-8 flex items-center justify-between bg-red-400 rounded max-w-3xl">
      <div class="flex items-center">
        ...
        <div v-if="$page.props.flash.error" class="py-4 text-white text-sm font-medium">{{ $page.flash.error }}</div>
        <div v-else class="py-4 text-white text-sm font-medium">
          <span v-if="Object.keys($page.flash.errors).length === 1">There is one form error.</span>
          <span v-else>There are {{ Object.keys($page.flash.errors).length }} form errors.</span>
        </div>
      </div>
      <button type="button" class="group mr-2 p-2" @click="show = false">
